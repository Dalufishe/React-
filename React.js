 ...
 * 
 * $ Ant Design
 * * Antd 是一門 UI/UX 設計語言, 一個設計和開發的橋樑, 並提供大規模組件庫進行前端開發
 * 
 * $ immutable.js
 * * 當我們想操作數據(本身是可變的)但又不能更改到原本的數據, 則:
 * * => 傳統拷貝方案
 * * => 轉不可變方案
 * 
 * ? 物件的拷貝
 ...
 * * 淺拷貝
 * * => 第一層的成功複製, 更下層仍是引用
 * * => 分散運算子, Object.assign等
 * * 深拷貝
 * * => 完全複製
 * * => JSON序列化和解析, 遞歸
 * 
 * ? Immutable
 * * Immutable(不可變物件)將"原始物件"轉為"不可變物件"
 * * 1. 修改不可變物件, 會回傳新的不可變物件, 不會改到原本的值 
 * * => 不會更改到原數據
 * * 2. 將可變物件轉為不可變物件, 不同於複製, 並不會重建複雜數據結構(取而代之外面套一層不可變類別的皮)
 * * => 相較拷貝性能高
 * 
 * * Map(): 將可變物件轉為不可變物件
 * * List(): 將可變陣列轉為不可變陣列
 * 
 * * [注]: 將可變物件轉為不可變物件僅能將轉換當前物件, 嵌套物件不適用, 需手動多加, 範圍如同拷貝中的淺拷貝
*/